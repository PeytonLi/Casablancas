...
test("plays every Charlie demo line from bundled audio", async () => {
  const played = [];
  const player = createCharlieVoicePlayer({
    fetchImpl: async () => {
      throw new Error("Charlie demo lines must not fetch");
    },
    audioFactory: (url) => ({
  ...
  await player.speak("Hey, I'm Charlie, here to help.");
  await player.speak("The Strokes are playing at Lands End right now.");
  await player.speak("Okay, follow me.");
  await player.speak("Ask me what's happening at the stage or where the restroom is.");

  assert.deepEqual(played, [
    "/public/audio/charlie-greeting.mp3",
    "/public/audio/charlie-stage-now.mp3",
    "/public/audio/charlie-toilet.mp3?v=charlie-voice-2",
    "/public/audio/charlie-capabilities.mp3",
  ]);
});
...
test("plays a bundled hard-coded answer without making a network request", async () => {
  let playedUrl = "";
  const player = createCharlieVoicePlayer({
    audioByText: { "Okay, follow me.": "/public/audio/charlie-toilet.mp3" },
    fetchImpl: async () => {
  ...
  assert.equal(playedUrl, "/public/audio/charlie-toilet.mp3");
});
