...
function festivalDestinationFeature(destination = NAVIGATION_ROUTE[0]) {
  return routeFeature([NAVIGATION_ROUTE[0], destination]);
}
...
export function navigateToPlace(id) {
  ...
  animationStartedAt = 0;
  progress = 0;
  source.setData(festivalDestinationFeature(place.coordinates));
  setDestinationRouteVisible(true);
  puckMarker.setLngLat(NAVIGATION_ROUTE[0]);
  map.fitBounds([NAVIGATION_ROUTE[0], place.coordinates], {
    padding: { top: 145, right: 48, bottom: 155, left: 48 },
    duration: 650,
    maxZoom: 17.2,
    pitch: 46,
    bearing: -12,
